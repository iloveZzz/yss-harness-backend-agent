...
| 稳定 ID | 产物 | 所属阶段 | 触发条件 |
|---|---|---|---|
| `artifact.technical-design` | 技术设计合同 | `stage.technical-design` | 后端技术设计影响；按确认的 DDD 或 MVC 架构产出。 |
...
| 稳定 ID | 证据 | 说明 |
|---|---|---|
| `evidence.technical-design-review` | 技术设计评审证据 | 按确认架构核验业务规则、关键场景、分层、事务与测试边界。 |
