...
后端仅负责当前业务切片的真实接口交付。战略原件在上游维护；Freeze 前收集前端消费需求。交付使用 `scripts/backend-delivery`，合同见 `docs/process/frontend-backend-delivery.md`。本仓 verification 通过只代表后端可交接，整体完成须统一管理方核验前端及端到端证据。
