...
- **Prototype** (HITL): Raise the fidelity of the discussion by making a cheap, rough, concrete artifact to react to — an outline, a rough take, a stub, or backend logic code via the /prototype skill. Product UI exploration returns to the design/frontend project. Links the prototype as an asset. Use when backend behavior, state or an interface assumption is the key question.
