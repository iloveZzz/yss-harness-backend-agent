...
In this template, **`harness-orchestrator` is the default orchestrator**. Native work-units own Spec, Slice Contract, implementation authorization, and verification gates.
...
2. **Branch — can you settle every question in conversation?** If a question needs a runnable answer (backend state, business logic, algorithm or interface behavior), detour through a prototype, bridged by **`/handoff`** in both directions (a prototype lives in its own directory, which is exactly what `/handoff` is for — see Phase boundaries):
...
- **`/prototype`** — a small, throwaway program that answers one design question: does this backend state model, algorithm or interface assumption hold. Product UI questions return to the design/frontend project. Throwaway is a constraint on how the code is written, not a promise to destroy it: the answer folds into the real code, and the prototype can be retained as a **primary source** with an evidence pointer; Git branch/commit actions require explicit user authorization. It's the detour in step 2 of the main flow, but reach for it any time a design question is hard to settle on paper.
