...
export function attachDesignPrerequisites(root, contract, { dataImpact = "not-applicable", apiImpact = "not-applicable" } = {}) {
  ...
  const apiDecision = {
    schema_version: 1, kind: "api-contract-decision", decision_id: `api-contract.${contract.project_name}`,
    decision_version: "v1", status: "approved", current_version: true, impact: apiImpact,
    assessment_ref: apiAssessment.ref, assessment_digest: apiAssessment.digest, evidence_refs: [apiEvidence.ref],
  };
  let openapi = null;
  if (apiImpact === "required") {
    ...
    openapi = write(root, `docs/.scratch/${slug}/api/${slug}.yaml`, [
      "openapi: 3.1.0", "info:", `  title: ${contract.project_name} API`, "  version: 1.0.0", "paths:",
      "  /health:", "    get:", "      operationId: getHealth", "      responses:", "        '204':", "          description: healthy", "",
    ].join("\n"));
    ...
    const validation = write(root, `docs/.scratch/${slug}/api/${slug}-validation.json`, {
      schema_version: 1, kind: "openapi-draft-validation", template: false, status: "passed",
      draft: { ref: openapi.ref, sha256: openapi.digest, oas_version: "3.1.0" },
      toolchain: { package: "@redocly/cli", version: "2.0.0", lockfile_ref: "pnpm-lock.yaml", command: `pnpm exec redocly lint ${openapi.ref}`, exit_code: 0, executed_at: "2026-09-14T00:00:00Z", evidence_ref: lintEvidence.ref },
      checks: { single_document: "passed", oas_3_1: "passed", ref_resolution: "passed", path_parameters: "passed", operation_ids: "passed", lint: "passed" },
    });
    const reviewEvidence = write(root, `docs/.scratch/${slug}/api/draft-review-evidence.md`, "独立 Draft Review 测试证据：无阻断项。\n");
    const reviewRecord = write(root, `docs/.scratch/${slug}/api/draft-review.json`, { schema_version: 1, kind: "openapi-draft-review", result: "approved", blocking_findings: [], draft_ref: openapi.ref, draft_digest: openapi.digest, evidence_refs: [reviewEvidence.ref] });
    Object.assign(apiDecision, {
      ...
      draft_review: { ref: reviewRecord.ref, digest: reviewRecord.digest, result: "approved", blocking_findings: [], draft_ref: openapi.ref, draft_digest: openapi.digest, evidence_refs: [reviewEvidence.ref] },
      freeze: { version: "v1", frozen_at: "2026-09-14T00:01:00Z", draft_ref: openapi.ref, draft_digest: openapi.digest },
    });
  } else apiDecision.reason = "当前机械后端骨架不新增或修改对外 API、消息契约或集成接口";
  ...
  const engineeringPackage = write(root, "engineering-contract-package.json", {
    schema_version: 1, kind: "engineering-contract-package", project_id: contract.project_name,
  ...
  contract.approval = { approval_ref: approval.file, approver: "role.test-agent", persisted_ref: contract.persisted_ref, current_version: contract.contract_version };
