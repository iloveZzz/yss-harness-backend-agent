...
export async function fixture(root, { technicalDesign = false, handoffVersion = 3, impacts = { ui: true, api: true, data: true, backend: true, frontend: true, cross_repo: false, high_risk: false } } = {}) {
 ...
 const snapshot={context_ref:'CONTEXT.md',context_schema_version:1,document_digest:source.document_digest,referenced_terms_digest:resolved.referenced_terms_digest,term_refs:['Global/Supplier']};
...
const visualBaselineRef = { baseline_id: sealedBaseline.baseline_id, version: sealedBaseline.version, digest: sealedBaseline.bundle.digest, status: "approved", persisted_ref: "source/visual-baseline-v1", manifest_ref: "visual-baseline.yaml", case_ids: sealedBaseline.cases.map((item) => item.case_id) };
 ...
 const strategy={...baseStrategy,schema_version:handoffVersion===4?3:2,domain_strategy_id:'domain-strategy.supplier',domain_version:'v1',status:'approved',contexts:baseStrategy.contexts,traceability_version:1,rule_catalog:[{rule_id:'rule.complete',statement:'申请材料必须完整',responsible_context:'SupplierManagement',status:'confirmed',...(handoffVersion===4?{evidence_refs:['evidence/offline.log']}:{})}],scenarios:[{scenario_id:'scenario.submit',actor:'supplier',preconditions:['draft'],commands:['Submit'],rules:['申请材料必须完整'],rule_refs:['rule.complete'],events:['Submitted'],success_results:['已提交'],failure_results:['缺材料拒绝'],responsible_context:'SupplierManagement',consumers:[],status:'confirmed',critical:true,...(handoffVersion===4?{evidence_refs:['evidence/offline.log']}:{})}],invariants:[{invariant_id:'invariant.complete',statement:'申请材料必须完整',rule_ref:'rule.complete',responsible_context:'SupplierManagement',scenario_refs:['scenario.submit'],verification_method:'test',...(handoffVersion===4?{evidence_refs:['evidence/offline.log']}:{})}],context_snapshot:snapshot,evidence_refs:['evidence/offline.log']};
 if(handoffVersion===4)strategy.downstream_mapping=[
 ...
 const stage={...read(new URL('./base-stage.json',import.meta.url)),schema_version:handoffVersion===4?3:2,stage_decision_id:'stage-decision.supplier',package_version:'v1',status:'approved',domain_strategy_ref:{domain_strategy_id:strategy.domain_strategy_id,domain_version:'v1',status:'approved',persisted_ref:'source/strategy.yaml',digest:digest(strategy)},context_snapshot:snapshot,unresolved_items:[],impact_assessment:impacts};
 if(handoffVersion===4){
   const record=(id,statement,source_refs)=>({id,statement,source_refs,evidence_refs:['evidence/offline.log'],status:'confirmed'});
   stage.success_criteria=[record('success-criterion.submitted','供应商申请成功提交',['scenario.submit'])];
   stage.test_seams=[record('test-seam.submit','supplier-application-submit',['rule.complete','scenario.submit'])];
   stage.confirmed_decisions=[record('decision.supplier-submit-owner','SupplierManagement 负责供应商提交',['rule.complete'])];
   stage.assumptions=[record('assumption.subject-available','供应商主体已存在',['scenario.submit'])];
 ...
 const handoff={schema_version:handoffVersion,handoff_id:'strategic-design-handoff.supplier',handoff_version:'v1',status:'approved',source:sourceRefs,source_context_snapshot:snapshot,context_delta:{added:resolved.terms.map(({term_ref,term,meaning,english_identifier,context_id,forbidden_aliases})=>({term_ref,term,meaning,english_identifier,context_id,forbidden_aliases})),updated:[],deprecated:[]},problem_and_business_outcomes:['提交供应商材料'],bounded_context_and_subdomain_map:['SupplierManagement'],context_map_and_translation_responsibility:['SupplierManagement'],ubiquitous_language_and_concept_candidates:['Global/Supplier'],scenarios_and_business_invariants:['rule.complete','scenario.submit'],tactical_design_questions:['一致性边界'],deferred_decisions_and_ownership:[],evidence_and_version_digests:['evidence/offline.log'],acceptance:['traceability'],package_export:{schema_version:1,approvals,additional_files:[],reference_map:{},prototype:{profile:'H2',preview_root:'preview',entry_ref:'preview/index.html',verification_ref:'evidence/offline.json',verification_digest:hash(readFileSync(path.join(root,'evidence/offline.json'))),source_digest:treeDigest(root,'prototype-src'),source_root:'prototype-src',lock_ref:'prototype-src/pnpm-lock.yaml'}}};
 if(handoffVersion===3){
   handoff.target_context_reconciliation={target_context_ref:'CONTEXT.md',reconciliation_schema_ref:'docs/process/schemas/context-reconciliation.schema.json',required_before_skill:technicalDesign?'yss-technical-design':'yss-tactical-design',status:'pending'};
   handoff.target={team:'downstream-rd-team',skill:technicalDesign?'yss-technical-design':'yss-tactical-design',output:technicalDesign?'technical-design-contract':'tactical-design-contract'};
 }else{
   const backendRequired=impacts.backend||impacts.api||impacts.data;
   const frontendRequired=impacts.frontend||impacts.ui;
   const route=(route_id,capability,activation,impact_refs,entry_work_unit,expected_outputs,dependencies)=>({route_id,capability,activation,impact_refs,entry_work_unit,expected_outputs,dependencies,context_reconciliation:{required:true,target_context_ref:'CONTEXT.md',schema_ref:'docs/process/schemas/context-reconciliation.schema.json'},...(activation==='not-applicable'?{reason:'批准的影响矩阵未命中该消费者能力',evidence_refs:['evidence/offline.log']}:{})});
   handoff.consumer_routes=[
     route('route.backend','backend-technical-design',backendRequired?'required':'not-applicable',['impact_assessment.backend','impact_assessment.api','impact_assessment.data'],'work-unit.technical-design',['technical-design-contract','backend-delivery'],[]),
     route('route.frontend','frontend-engineering-design',frontendRequired?'required':'not-applicable',['impact_assessment.frontend','impact_assessment.ui'],'work-unit.frontend-engineering-design',['frontend-strategic-preflight','frontend-engineering-design','frontend-delivery-acceptance'],backendRequired?['route.backend']:[]),
