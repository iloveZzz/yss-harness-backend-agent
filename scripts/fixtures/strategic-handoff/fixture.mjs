...
const visualBaselineRef = { baseline_id: sealedBaseline.baseline_id, version: sealedBaseline.version, digest: sealedBaseline.bundle.digest, status: "approved", persisted_ref: "source/visual-baseline-v1", manifest_ref: "visual-baseline.yaml", case_ids: sealedBaseline.cases.map((item) => item.case_id) };
 ...
 if(routed)strategy.downstream_mapping=[
 ...
 if(handoffVersion===3){
   handoff.target_context_reconciliation={target_context_ref:'CONTEXT.md',reconciliation_schema_ref:'.template-spec/process/schemas/context-reconciliation.schema.json',required_before_skill:technicalDesign?'yss-technical-design':'yss-tactical-design',status:'pending'};
   ...
   const route=(route_id,capability,activation,impact_refs,entry_work_unit,expected_outputs,dependencies)=>({route_id,capability,activation,impact_refs,entry_work_unit,expected_outputs,dependencies,context_reconciliation:{required:true,target_context_ref:'CONTEXT.md',schema_ref:'.template-spec/process/schemas/context-reconciliation.schema.json'},...(activation==='not-applicable'?{reason:'批准的影响矩阵未命中该消费者能力',evidence_refs:['evidence/offline.log']}:{})});
